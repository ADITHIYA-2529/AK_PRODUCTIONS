...
export default function Gallery() {
    ...
    Promise.all([getAllGallery(), getSiteSettings()])
      .then(([data, settingsData]) => {
        ...
        if (data && data.length > 0) {
          const mapped = data
            .filter((item: any) => item.image || item.videoUrl)  // include image OR video items
            .map((item: any) => ({
              id: item._id,
              ...
              altText: item.altText || item.title || 'AK Productions Gallery',
  ...
  return (
    <>
                  ...
                  >
                    {item.mediaType === 'video' ? (
                      <video
                        ...
                        className={`w-full object-cover transition-transform duration-600 group-hover:scale-105 ${item.aspectRatio === 'portrait' ? 'aspect-[3/4]' :
                      ...
                      <img
                        src={item.src}
                        alt={item.altText || item.title || 'AK Productions Event Gallery'}
        ...
        {/* Lightbox */}
        <GalleryLightbox
          images={visible.map(i => i.mediaType === 'video' ? i.videoUrl : i.src)}
          titles={visible.map(i => i.title)}
          mediaTypes={visible.map(i => i.mediaType)}
